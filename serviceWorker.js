...
self.addEventListener('activate', (event) => {
  event.waitUntil(caches
    ...
    .then(keys => Promise.all( // Return a promise that resolves when outdated caches are deleted.
      keys
        .filter(key => // Filter over the keys array.
        ...
        .map(key => // Map over the filtered array.
